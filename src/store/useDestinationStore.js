...
export const useDestinationStore = create()(persist((set, get) => ({
    destinations: initialDestinations,
    addDestination: (destination) => set((state) => ({ destinations: [...state.destinations, destination] })),
    updateDestination: (id, updatedData) => set((state) => ({
        destinations: state.destinations.map((d) => d.id === id ? { ...d, ...updatedData } : d)
    })),
